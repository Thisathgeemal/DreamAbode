<x-agent-layout>
    ...
    <div class="w-full px-8 py-6 bg-[#161616] rounded-lg text-left mx-auto shadow-md mb-6">
        <h2 class="text-2xl text-white font-bold">Assigned Projects</h2>
        <p class="text-sm text-gray-300 mt-1">Review all your approved project assignments.</p>
    </div>

    <!-- Main Card -->
    <div class="w-full p-8 bg-white rounded-lg text-left mx-auto shadow-md mb-6">
        <div id="assigned-project"
    ...
    @push('scripts')
        <script>
            ...
            async function fetchAssignedProjects() {
                const container = document.getElementById('assigned-project');
                container.innerHTML = `
                <p class="flex justify-center items-center text-center text-gray-500 text-lg mt-10">
                    Loading assigned projects, please wait...
                </p>`;
                ...
                try {
                    ...
                    const projects = response.data.agent_projects.approved;
                    container.innerHTML = '';
