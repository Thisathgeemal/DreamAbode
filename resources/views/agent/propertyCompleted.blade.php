<x-agent-layout>
    ...
    <div class="w-full px-8 py-6 bg-[#161616] rounded-lg text-left mx-auto shadow-md mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-2xl text-white font-bold">Deal Completed Property</h2>
                <p class="text-sm text-gray-300 mt-1">Review all your completed property deals.</p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full p-8 bg-white rounded-lg text-left mx-auto shadow-md mb-6">
        <div id="agent-completed-property"
    ...
    @push('scripts')
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                fetchAgentCompletedProperties();
            });

            // Fetch agent completed properties
            async function fetchAgentCompletedProperties() {
                const container = document.getElementById('agent-completed-property');
                container.innerHTML = `
                ...
                try {
                    ...
                    // Only agent completed properties
                    const properties = response.data.agent_properties.completed;
                    container.innerHTML = '';

                    if (!properties || properties.length === 0) {
                        container.innerHTML = `
                        <p class="flex justify-center items-center text-center text-green-500 text-lg">
                            No completed property deals found.
                        </p>`;
                        return;
                    }
                    ...
                    properties.forEach(prop => {
                        ...
                        // Actions for completed properties
                    ...
                    console.error('Error fetching agent completed properties:', error);
