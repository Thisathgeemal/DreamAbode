<x-admin-layout>
    ...
    <div class="w-full px-8 py-6 bg-[#161616] rounded-lg text-left mx-auto shadow-md mb-6">
        <h2 class="text-2xl text-white font-bold">Agent Management</h2>
        <p class="text-sm text-gray-300 mt-1">Keep your users organized and your system secure.</p>
    </div>
    ...
    <div class="w-full p-8 bg-white rounded-lg text-left mx-auto shadow-md mb-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div class="flex flex-col sm:flex-col md:flex-row gap-3 w-full md:w-auto">
                ...
                <button type="button" onclick="deleteSelectedAgents()"
            ...
            <div class="flex items-center space-x-3 w-full md:w-auto">
                <form class="relative flex-1" onsubmit="searchAgents(event)">
                ...
                <form target="_blank" class="relative group" onsubmit="exportAgents(event)">
                    @csrf
        ...
        <div class="overflow-auto max-w-[300px] md:max-w-7xl mt-7 border rounded shadow-md">
            <table class="min-w-full bg-white shadow-md rounded-lg border text-base border-green-400" id="agent-table">
                ...
                <tbody>
                    <!-- Agent data will be populated by JS -->
                </tbody>
            </table>
        </div>
    ...
    <div id="agentModal" role="dialog" aria-modal="true"
        ...
        <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-[0_0_15px_4px_rgba(92,255,171,0.4)]">
            <h2 class="text-md md:text-3xl font-bold text-center mb-4" id="modalHeader">Create Agent Account</h2>
        ...
        <script>
            ...
            function loadAgents(page = 1, search = '') {
                ...
                axios.get(`/api/agents?page=${page}&search=${search}`, {
                    ...
                    .then(res => {
                        // let agents = res.data.data ?? res.data;
                ...
                agents.forEach(agent => {
                    html += `
                        <tr class="hover:bg-gray-100 transition duration-200">
                            <td class="py-3 px-4 border-b"><input type="checkbox" name="selector[]" class="h-4 w-4" value="${agent.id}"></td>
                            <td class="py-3 px-4 border-b">${agent.name}</td>
                            <td class="py-3 px-4 border-b">${agent.email}</td>
                            <td class="py-3 px-4 border-b">${agent.mobile_number ?? ''}</td>
                            <td class="py-3 px-4 border-b">${agent.address ?? ''}</td>
                            <td class="py-3 px-4 border-b border-gray-200">
                                <button onclick="openEditModal(${agent.id})" class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-green-100 hover:bg-green-200 transition duration-200 text-green-500 hover:text-green-700" title="Edit">
            ...
            function renderPagination(data) {
                ...
                for (let i = 1; i <= data.last_page; i++) {
                    html +=
                        `<button onclick="loadAgents(${i}, '${lastSearch}')" class="px-3 py-1 rounded ${i === currentPage ? 'bg-green-500 text-white' : 'bg-gray-200'} mr-1">${i}</button>`;
                }
            ...
            function openModal() {
                ...
                document.getElementById('modalHeader').textContent = 'Create Agent Account';
            ...
            function openEditModal(id) {
                axios.get(`/api/agents/${id}`, {
                    ...
                    .then(res => {
                        ...
                        document.getElementById('mobile_number').value = agent.mobile_number ?? '';
                        document.getElementById('address').value = agent.address ?? '';
                        document.getElementById('modalHeader').textContent = 'Edit Agent Account';
            ...
            document.getElementById('agentForm').addEventListener('submit', function(e) {
                ...
                if (id) {
                    ...
                    axios.post(`/api/agents/${id}`, formData, {
                            headers: {
                                'Authorization': `Bearer ${token}`,
                                'Accept': 'application/json',
                        ...
                        .then(res => {
                            ...
                            closeModal();
                            loadAgents(currentPage, lastSearch);
            ...
            function deleteSelectedAgents() {
                ...
                if (!selected.length) return showInfo('Please select at least one agent to delete.');

                Swal.fire({
                    title: 'Are you sure?',
                    text: "Do you really want to delete selected agent(s)?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        // Loop through each selected agent and delete individually
                        for (const id of selected) {
                            try {
                                ...
                                const msg = err.response?.data?.error || `Failed to delete agent with ID ${id}`;
                                showError(msg);
                                return;
                            }
                        }
                        ...
                        loadAgents(currentPage, lastSearch);
                        showSuccess('Selected agent(s) deleted successfully!');
                    }
                });
            }
