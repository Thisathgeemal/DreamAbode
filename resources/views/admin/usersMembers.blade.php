<x-admin-layout>
    ...
    <div class="w-full px-8 py-6 bg-[#161616] rounded-lg text-left mx-auto shadow-md mb-6">
        <h2 class="text-2xl text-white font-bold">Member Management</h2>
    ...
    <div class="w-full p-8 bg-white rounded-lg text-left mx-auto shadow-md mb-6">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div class="flex items-center space-x-3 w-full md:w-auto">
                <form class="relative flex-1" onsubmit="searchMembers(event)">
                ...
                <form target="_blank" class="relative group" onsubmit="exportMembers(event)">
                    @csrf
        ...
        <div class="overflow-auto max-w-[300px] md:max-w-7xl mt-7 border rounded shadow-md">
            <table class="min-w-full bg-white shadow-md rounded-lg border text-base border-green-400" id="member-table">
                ...
                <tbody>
                    <!-- Member data will be populated by JS -->
                </tbody>
            </table>
        </div>
        ...
        <script>
            ...
            function loadMembers(page = 1, search = '') {
                ...
                axios.get(`/api/members?page=${page}&search=${search}`, {
                    ...
                    .then(res => {
                        // let members = res.data.data ?? res.data;
                ...
                members.forEach(member => {
                    html += `
                        <tr class="hover:bg-gray-100 transition duration-200">
                            <td class="py-3 px-4 border-b"><input type="checkbox" name="selector[]" class="h-4 w-4" value="${member.id}"></td>
                            <td class="py-3 px-4 border-b">${member.name}</td>
                            <td class="py-3 px-4 border-b">${member.email}</td>
                            <td class="py-3 px-4 border-b">${member.mobile_number ?? ''}</td>
                            <td class="py-3 px-4 border-b">${member.address ?? ''}</td>
                            <td class="py-3 px-4 border-b border-gray-200">
                                <button onclick="openEditModal(${member.id})" class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-green-100 hover:bg-green-200 transition duration-200 text-green-500 hover:text-green-700" title="Edit">
            ...
            function renderPagination(data) {
                ...
                for (let i = 1; i <= data.last_page; i++) {
                    html +=
                        `<button onclick="loadMembers(${i}, '${lastSearch}')" class="px-3 py-1 rounded ${i === currentPage ? 'bg-green-500 text-white' : 'bg-gray-200'} mr-1">${i}</button>`;
                }
            ...
            function openModal() {
                ...
                document.getElementById('memberModel').classList.remove('hidden');
                document.getElementById('memberModel').classList.add('flex');
            }
            ...
            function openEditModal(id) {
                axios.get(`/api/members/${id}`, {
                    ...
                    .then(res => {
                        ...
                        document.getElementById('mobile_number').value = member.mobile_number ?? '';
                        document.getElementById('address').value = member.address ?? '';
                        document.getElementById('modalHeader').textContent = 'Edit Member Account';
            ...
            function closeModal() {
                document.getElementById('memberModel').classList.add('hidden');
                document.getElementById('memberModel').classList.remove('flex');
            }
            ...
            document.getElementById('memberForm').addEventListener('submit', function(e) {
                ...
                if (id) {
                    ...
                    axios.post(`/api/members/${id}`, formData, {
                            headers: {
                                'Authorization': `Bearer ${token}`,
                                'Accept': 'application/json',
                        ...
                        .then(res => {
                            ...
                            closeModal();
                            loadMembers(currentPage, lastSearch);
            ...
            function deleteSelectedMembers() {
                ...
                if (!selected.length) return showInfo('Please select at least one member to delete.');

                Swal.fire({
                    title: 'Are you sure?',
                    text: "Do you really want to delete selected member(s)?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        // Loop through each selected member and delete individually
                        for (const id of selected) {
                            try {
                                ...
                                const msg = err.response?.data?.error || `Failed to delete member with ID ${id}`;
                                showError(msg);
                                return;
                            }
                        }
                        ...
                        loadMembers(currentPage, lastSearch);
                        showSuccess('Selected member(s) deleted successfully!');
                    }
                });
            }
